...
% algorithm
%% get path to the data_import script
...
hop_dist = load(fullfile(pathstr,'d_hop_distribution.mat'),'pd');
hop_dist = hop_dist.pd;

%Distribution of positive load/total load - 1/N (tLocation-Scale)
Pload_dist = load(fullfile(pathstr,'Pload_distribution.mat'),'pd');
...
pmax_coeff = load(fullfile(pathstr,'pmax_coeff'),'maxp_dhop');
...
coincident_inom_ratio = load(fullfile(pathstr,'coincident_inom_ratio_cdf.mat'),'coincident_inom_ratio');
coincident_inom_ratio = coincident_inom_ratio.coincident_inom_ratio;

%Per unit voltage drop distribution for cables
vdrop_dist = load(fullfile(pathstr,'vdrop_distribution.mat'),'pd_est');
vdrop_dist = vdrop_dist.pd_est;

%Cable Length Distribution (modified Cauchy)
length_dist = load(fullfile(pathstr,'length_distribution.mat'),'cauchyMod','cauchyHat');
...
length_max = load(fullfile(pathstr,'length_max.mat'),'fitobject');
